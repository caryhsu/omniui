...
MODULE_PATH="$(to_win_path "$ROOT/demo/java/input-app/target/classes");$(to_win_path "$JAVA_FX_CONTROLS");$(to_win_path "$JAVA_FX_GRAPHICS")"
java -p "$MODULE_PATH" -m dev.omniui.demo.input/dev.omniui.demo.input.InputDemoApp "$@"
