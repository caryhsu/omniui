...
MODULE_PATH="$(to_win_path "$ROOT/demo/java/core-app/target/classes");$(to_win_path "$ROOT/java-agent/target/omniui-java-agent-0.1.0-SNAPSHOT.jar");$(to_win_path "$JAVA_FX_CONTROLS");$(to_win_path "$JAVA_FX_GRAPHICS");$(to_win_path "$JAVA_FX_BASE");$(to_win_path "$GSON_JAR")"
java "-javaagent:$(to_win_path "$AGENT_JAR")=port=48100" -p "$MODULE_PATH" -m dev.omniui.demo.core/dev.omniui.demo.core.CoreDemoApp "$@"
