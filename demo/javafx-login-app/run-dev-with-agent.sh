...
MODULE_PATH="$(to_win_path "$ROOT/demo/javafx-login-app/target/classes");$(to_win_path "$ROOT/java-agent/target/omniui-java-agent-0.1.0-SNAPSHOT.jar");$(to_win_path "$JAVA_FX_CONTROLS");$(to_win_path "$JAVA_FX_GRAPHICS")"
java "-javaagent:$(to_win_path "$AGENT_JAR")" -p "$MODULE_PATH" -m dev.omniui.demo/dev.omniui.demo.LoginDemoApp "$@"
