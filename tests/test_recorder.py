...
class RecorderRunTests(unittest.TestCase):
    ...
    def _make_app(self):
        ...
        # Suppress the startup scan thread so GUI tests stay deterministic.
    ...
    def test_exec_script_strips_recorder_header(self) -> None:
        ...
        # OmniUI.connect() is no longer stripped — it runs through the stub so
        # that kwargs such as screenshot_mode are forwarded to the existing client.
        self.assertIn("OmniUI.connect()", stripped)
        self.assertNotIn("# Generated by OmniUI Recorder", stripped)
        self.assertIn("_sentinel = True", stripped)

    def test_fetch_and_show_assertion_menu_ignores_request_errors(self) -> None:
        from unittest.mock import patch

        app = self._make_app()
        app._client = type("Client", (), {"session_id": "s1", "base_url": "http://127.0.0.1:48100"})()

        with patch("omniui.recorder.gui.urllib.request.urlopen", side_effect=OSError("boom")), \
             patch.object(app, "_show_assertion_menu") as mock_show:
            app._fetch_and_show_assertion_menu(10.5, 20.5)

        mock_show.assert_not_called()

    def test_fetch_and_show_assertion_menu_skips_when_no_available_assertions(self) -> None:
        from unittest.mock import patch
        ...
        class _Resp:
            ...
            def read(self):
                return b'{"fxId":"statusLabel","availableAssertions":[]}'

        with patch("omniui.recorder.gui.urllib.request.urlopen", return_value=_Resp()), \
             patch.object(app, "_show_assertion_menu") as mock_show:
            app._fetch_and_show_assertion_menu(10.5, 20.5)

        mock_show.assert_not_called()

    def test_fetch_and_show_assertion_menu_uses_pointer_fallback_coordinates(self) -> None:
        from unittest.mock import patch
        ...
        class _Resp:
            ...
            def read(self):
                return (
                    b'{"fxId":"statusLabel","nodeType":"Label","currentText":"Ready",'
                    b'"availableAssertions":["verify_text"]}'
                )

        with patch("omniui.recorder.gui.urllib.request.urlopen", return_value=_Resp()), \
             patch.object(app.root, "winfo_pointerx", side_effect=RuntimeError("no pointer")), \
             patch.object(app.root, "winfo_pointery", side_effect=RuntimeError("no pointer")), \
             patch.object(app, "_show_assertion_menu") as mock_show:
            app._fetch_and_show_assertion_menu(10.5, 20.5)

        mock_show.assert_called_once()
        ctx, available, x, y = mock_show.call_args.args
        self.assertEqual(ctx["fxId"], "statusLabel")
        self.assertEqual(available, ["verify_text"])
        self.assertEqual((x, y), (0, 0))

    def test_polling_loop_routes_right_click_events_and_appends_partial_script(self) -> None:
        from omniui.core.engine import OmniUIClient
        from unittest.mock import patch

        app = self._make_app()
        client = OmniUIClient(base_url="http://127.0.0.1:48100", session_id="s1")
        client._wait_injection = False
        client._wait_timeout = 5.0
        app._client = client
        app._polling = True

        raw_batch = [
            {"type": "right_click", "fxId": "statusLabel", "availableAssertions": ["verify_text"]},
            {"type": "click", "fxId": "saveButton", "text": "", "nodeType": "Button", "nodeIndex": 0, "timestamp": 1.0},
        ]
        ...
        client.poll_events = fake_poll_events

        with patch("time.sleep", return_value=None), \
             patch.object(app.root, "after", side_effect=lambda _d, fn, *a: fn(*a)), \
             patch.object(app, "_show_assertion_menu_from_event") as mock_menu, \
             patch.object(app, "_append_script_lines") as mock_append:
            app._polling_loop()

        mock_menu.assert_called_once_with(raw_batch[0], ["verify_text"])
        mock_append.assert_called_once()
        self.assertIn('client.click(id="saveButton")', mock_append.call_args.args[0])

    def test_polling_loop_disconnects_after_three_consecutive_errors(self) -> None:
        from omniui.core.engine import OmniUIClient
        from unittest.mock import patch
        ...
        client.poll_events = always_fail

        with patch("time.sleep", return_value=None), \
             patch.object(app.root, "after", side_effect=lambda _d, fn, *a: fn(*a)), \
             patch.object(app, "_on_remote_app_closed") as mock_closed:
            app._polling_loop()

        mock_closed.assert_called_once()

    def test_stop_recording_shows_error_when_client_stop_fails(self) -> None:
        from unittest.mock import MagicMock, patch

        app = self._make_app()
        app._client = MagicMock()
        app._client.stop_recording.side_effect = RuntimeError("stop failed")
        app._polling = False

        with patch("omniui.recorder.gui.messagebox.showerror") as mock_error:
            app._stop_recording()
