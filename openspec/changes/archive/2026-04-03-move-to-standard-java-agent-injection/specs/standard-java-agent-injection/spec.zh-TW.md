...
- **THEN** Java agent 在 target JVM 內啟動，且不要求 target app source 直接引用 OmniUI class

### Requirement: Agent-owned control server
系統 SHALL 由 Java agent 在自身內部啟動並擁有 OmniUI HTTP control server，而不是由 target app bootstrap code 啟動。
...
系統 SHALL 從 injected Java agent 端發現 JavaFX runtime state 並註冊 automation targets，而不要求 app code 呼叫 `registerStage`、`registerScene` 或其他 OmniUI bridge API。
